            ...
            <thead>
              <tr>
                ...
                {COMPETITORS.map((competitor) => (
                  <th key={competitor} className='text-center p-2'>
                    {competitor}
                  </th>
                ))}
                {METRICS.map((metric) => (
            ...
            <tbody>
              {references.map((ref, index) => (
                <tr key={index} className='border-t'>
                  <td
                    className='p-2 flex flex-row items-center gap-2 cursor-pointer'
                  ...
                  {COMPETITORS.map((competitor) => (
                    <td key={competitor} className='text-center p-2'>
                      {!analysisResults[ref.link] ? '⏳' : analysisResults[ref.link][competitor] ? '✅' : '❌'}
                    </td>
                  ))}
                  {METRICS.map((metric) => (
