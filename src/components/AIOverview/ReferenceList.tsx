...
export default function ReferenceList({ references }: ReferenceListProps) {
  ...
  const displayedReferences = isExpanded ? references : references.slice(0, DEFAULT_DISPLAY_COUNT);

  return (
    <div className='space-y-4'>
      <Card className='bg-[#202124] p-4 space-y-4 gap-0'>
        {displayedReferences.map((reference, index) => (
          <a key={index} href={reference.link} target='_blank' rel='noopener noreferrer' className='block'>
            <div className='group rounded-lg hover:bg-white/5 transition-colors p-3 cursor-pointer'>
              <div className='flex items-start gap-3'>
                <div className='flex-1 min-w-0'>
                  <div className='flex items-center gap-2 mb-1'>
                    <h3 className='text-white text-sm font-medium truncate'>{reference.title}</h3>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger>
                          <span
                  ...
                  <p className='text-gray-400 text-sm line-clamp-2'>{reference.snippet}</p>
